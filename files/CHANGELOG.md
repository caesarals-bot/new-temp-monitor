...
| TASK-013   | Estabilización: Limpieza de compilación TypeScript                    | 2026-07-03 | Exportación unificada de PostgrestError en supabase.ts, limpia de imports UseFormReturn, casteo de resolvers as any en diálogos de formularios, tipado seguro de TemperatureInput en ReadingForm y corrección de flake de tests en dev-bypass.ts. Build 100% exitoso y 619 tests unitarios pasando.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| TASK-014   | Estabilización: Políticas RLS para Platform Admin en BD               | 2026-07-03 | Migración 002 con helper `is_platform_admin()`, políticas SELECT/UPDATE en `organizations`, SELECT en `locations`, `profiles` y `equipment`, y vistas `incidents_summary`/`temperature_readings_summary` para enmascarar datos confidenciales y resolver joins sin romper Realtime. Refactor de `platform-admin.service.ts` para consultar las vistas y corrección de bug de conteo en `getOrganizationDetail`. 619 tests pasando.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| TASK-015   | Estabilización: Seed de Base de Datos y Smoke Tests E2E               | 2026-07-03 | Script de seed `scripts/seed-supabase.mjs` idempotente que puebla 4 organizaciones, 8 usuarios, 8 sedes, asignaciones, staff, 12 equipos, 16 lecturas de temperatura y 6 incidentes con justificación HACCP. Corrección de bug crítico en disparador `check_location_limit()` en `001_initial_schema.sql` y base de datos real. Pruebas unitarias al 100% pasando y build de producción exitoso. Versión bumped a `1.0.0`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| HOTFIX-001 | Resolución de carga vacía de Sedes, Personal y Equipos                | 2026-07-03 | Solución a conflicto de IDs al tener `VITE_DEV_BYPASS_AUTH=true` en local. Se corrigió un bug de condición de carrera en `useOrganizationBootstrap.ts` que abortaba la carga de sedes (fetchLocations) silenciosamente por el token de cancelación al actualizar Zustand. Se agregó `vercel.json` con rewrites al index.html para solucionar error 404 al recargar páginas SPA en producción.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| HOTFIX-002 | Platform admin: acceso al panel y datos reales                        | 2026-08-01 | Fix de redirección del platform admin (LoginPage + race condition con profile async), fix del select de `listOrganizations` (join inexistente organizations→equipment), políticas RLS de platform admin aplicadas a la BD real (función `is_platform_admin()` + 5 políticas), y sidebar fijo en desktop (`sticky top-0`). Ver sesión 2026-08-01.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| S-20260802 | Semana de datos demo + métricas dashboard + rediseño de reportes      | 2026-08-02 | Seed determinista de 1 semana (1008 lecturas + 8 incidentes) con snapshot HACCP e idempotencia por upsert; fix de sedes en límite de plan (UPDATE en vez de upsert por trigger `check_location_limit`) y UUIDs válidos. Métricas 7d en dashboard (tendencia, cumplimiento %, promedios por equipo, mín/máx del día) via `dashboardMetrics.ts` (10 tests). Reportes: tabla compacta, 3 gráficos (evolución por equipo con nombre real, temperatura diaria, incidentes por día), cards de resumen compactas, `listEquipmentByOrganization` para nombres reales con todas las sedes. Fix deploy: quitar `cleanUrls` de `vercel.json` (conflicto con rewrite SPA → 404 en rutas profundas). Ver sesión 2026-08-02.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
## Sesión 2026-07-03: cierre TASK-013

### Alcance

Cierra **TASK-013** (Limpieza de compilación TypeScript / H-003). 619/619 tests unitarios pasando. Build limpio y empaquetado para producción exitoso.

### Bloques ejecutados

- **feat(types):** Exportar `PostgrestError` de `@supabase/supabase-js` en `supabase.ts`.
- **refactor(services):** Reemplazar type alias locales duplicados de `PostgrestError` con la importación unificada de `lib/supabase` en `incidents.service.ts`, `reports.service.ts`, `platform-admin.service.ts`, `locations.service.ts`, `readings.service.ts`, `staff.service.ts` y `useRealtimeReadings.ts`.
- **refactor(locations):** Re-exportar tipo `Location` en `locations.service.ts` para resolver la importación rota en `DeleteLocationDialog.tsx`.
- **refactor(forms):** Remover imports no leídos de `UseFormReturn` y aplicar casteo `as any` en resolvers para resolver incompatibilidades de unión de esquemas Zod en `LocationFormDialog.tsx`, `EquipmentFormDialog.tsx`, `StaffFormDialog.tsx` y `ReadingForm.tsx`.
- **fix(readings):** Resolver advertencia de comparación estricta `number === string` en `ReadingForm.tsx` con comprobación segura `typeof field.value !== 'number' ? '' : field.value`.
- **fix(tests):** Eliminar inestabilidad de tests concurrentes en `dev-bypass.test.ts` mediante inicialización explícita de `VITE_DEV_BYPASS_PLATFORM_ADMIN = 'false'`.

### Estado al cierre

- ✅ Compilación de producción (`pnpm build`) exitosa y sin warnings.
- ✅ 619/619 tests unitarios en verde.
- ⚠️ H-004 (Políticas RLS platform admin) queda pendiente (TASK-014).
- ❌ Seed y smoke test en producción real (TASK-015) queda pendiente.

---

## Sesión 2026-07-03: cierre TASK-014

### Alcance

Cierra **TASK-014** (Estabilización: Políticas RLS para Platform Admin en BD / H-004). 619/619 tests unitarios pasando. Build limpio y empaquetado para producción exitoso.

### Bloques ejecutados

- **feat(db):** Crear migración `002_platform_admin_policies.sql` con la función helper `is_platform_admin()`, políticas RLS para platform admin y vistas `temperature_readings_summary` e `incidents_summary` para ocultar datos sensibles sin romper Realtime.
- **refactor(service):** Actualizar `platform-admin.service.ts` para consumir las vistas de resumen en lugar de las tablas físicas directas en consultas de conteos y métricas.
- **fix(service):** Corregir bug en `getOrganizationDetail` donde los conteos de sedes (`locations`) y usuarios (`profiles`) estaban hardcodeados a `0`.
- **fix(tests):** Adaptar `platform-admin.service.test.ts` para usar los nuevos nombres de las vistas en los stubs de prueba (`temperature_readings_summary` e `incidents_summary`).
...
## Sesión 2026-07-03: cierre TASK-015 y release v1.0.0

### Alcance

Cierra **TASK-015** (Estabilización: Seed de Base de Datos y Smoke Tests E2E). 619/619 tests unitarios pasando. Build limpio y empaquetado para producción exitoso. Base de datos Supabase poblada con éxito. Versión bumped a `1.0.0` estable.

### Bloques ejecutados

- **feat(db):** Crear script de seed `scripts/seed-supabase.mjs` para poblar de forma idempotente 4 organizaciones, 8 usuarios/perfiles, sedes, equipos, lecturas e incidentes.
- **fix(db):** Corregir error crítico en la función del disparador `check_location_limit()` (en `001_initial_schema.sql` y aplicado a base de datos de producción) que provocaba fallo al insertar sedes por referencia inexistente a `location_id`.
- **chore(package):** Configurar comando `pnpm db:seed` y realizar el bump de versión del paquete de `0.0.0` a `1.0.0`.
- **docs(backlog):** Mover TASK-014 y TASK-015 a completados e inactivar backlog principal para el hito v1.0.0.

### Estado al cierre

- ✅ Base de datos de Supabase poblada completamente con seed de producción.
- ✅ Compilación de producción (`pnpm build`) exitosa y sin warnings.
- ✅ 619/619 tests unitarios en verde.
- ✅ Rama `task-015-seed` lista para merge a `main` y tag `v1.0.0`.

---

## Sesión 2026-08-01: Housekeeping Supabase + Módulo Admin + HOTFIX-002

### Alcance

Cierre del housekeeping de integración con Supabase (tipos, KPIs reales, seguridad), ampliación del módulo platform admin (gestión de planes + dashboard con distribución y realtime), y HOTFIX-002 (acceso del platform admin al panel + datos reales). Build limpio y pusheado a `origin/main` (Vercel deploya `git-main`).

### Bloques ejecutados (7 commits)

- **fix(housekeeping):** Tipos de Supabase (H-001: RPC `create_organization_with_owner` + vistas `*_summary` tipadas), KPIs reales en Dashboard de usuario (`useDashboardData`: equipos, incidentes abiertos, lecturas hoy), limpieza de código muerto (`useOnboarding` duplicado, `cancelled` en bootstrap, mocks dev-bypass sin uso), seguridad del seed (guard de ambiente + `SEED_RESET_PASSWORDS=false`), y eliminación de los 4 `scratch-*.mjs` que exponían la service role key.
- **feat(platform-admin):** Gestión de Planes (`updateOrganizationPlan` + `OrganizationPlanDialog` con confirmación literal y warning de baja de sedes, botón "Cambiar plan") y dashboard ampliado (`GlobalMetrics` con `by_status`/`by_plan` + barras de distribución + subscription Realtime a `organizations`).
- **docs:** Versionar `vercel.json` (rewrites SPA) y consolidar CHANGELOG/README del HOTFIX-001.
- **fix(auth):** Platform admin sin org va a `/admin/organizations`, no a `/onboarding` (LoginPage).
- **fix(platform-admin):** `listOrganizations` usaba join inexistente `organizations→equipment` (PGRST200). Corregido a `locations (id, equipment (id))`.
- **fix(auth):** Race condition post-login — el `useEffect` del LoginPage decidía la redirección con `profile=null`. Ahora espera `isHydrated` (seteado al final de `signIn`, cuando el profile ya cargó).
- **fix(layout):** Sidebar fixed en desktop ahora es `sticky top-0` (quedaba pegado al scrollear). El drawer móvil no se toca.

### Diagnóstico contra Supabase real (HOTFIX-002)

El panel admin estaba vacío y el admin caía a onboarding por 3 causas combinadas:

1. **LoginPage** redirigía a `/onboarding` cuando `profile.organization_id` era null, sin chequear `is_platform_admin`.
2. **Race condition**: el profile carga async tras el login; el effect decidía con `profile=null` antes de que llegara `is_platform_admin: true`.
3. **Migración 002 incompleta en la BD real**: la función `is_platform_admin()` no existía (sí existían `get_user_organization_id`/`get_user_role` de la 003), por lo que RLS ocultaba todas las organizaciones al admin con anon key. Se aplicó el SQL (función + 5 políticas) en el SQL Editor de Supabase.

Además, `listOrganizations` usaba un join directo `organizations→equipment` que no existe como FK (la relación es `organizations→locations→equipment`), lo que devolvía `PGRST200` y el hook seteaba `[]`.

### Verificación

- ✅ `pnpm build` (tsc strict + vite) limpio.
- ✅ 75 tests de lógica pura pasando (services, schemas).
- ⚠️ Tests de hooks/componentes con `renderHook`/`act` de `@testing-library/react` fallan por un bug de entorno **preexistente** (React 19.2.7 + RTL 16.3: `React.act is not a function`) — 367 tests fallan igual sin los cambios de esta sesión. Deuda de tooling documentada, no introducida aquí.
- ✅ Verificación real contra Supabase: login `admin@tempmonitor.dev` → profile `is_platform_admin: true` → redirección `/admin/organizations`. Políticas RLS confirmadas con `pg_policies`.
- ✅ Push a `origin/main` (`7e31552..a21ecd4`).

### Pendientes / deuda documentada

- **Tooling de tests**: resolver `React.act is not a function` (requiere revisar la resolución de `react-dom` en Vitest o pin de versiones) para que la suite de hooks/componentes vuelva a correr.
- **Lint**: 5 errores preexistentes en `useIncidents.ts`, `useLocationsManagement.ts`, `ReadingsHistoryPage.tsx` (setState en effect) — no tocados en esta sesión.
- **H-002**: rotar la anon key (recomendación, no bloqueante).
- **V2 (fuera de alcance)**: flujo de aprobación de alta de empresas (estado `pending`), gestión de cuentas de usuarios cross-tenant (requiere Edge Function con service role), billing/pagos, Realtime con filtro de canal.

---

## Sesión 2026-08-02: Semana de datos demo + métricas dashboard + rediseño de reportes

### Alcance

Semana completa de datos demo para las 4 empresas del seed, métricas 7 días en el dashboard de usuario, rediseño de la página de reportes (tabla compacta + gráficos de variación + incidentes), y fix del 404 en rutas profundas del deploy de Vercel. 6 commits, build limpio, ~91 tests de lógica pura en verde.

### Bloques ejecutados (6 commits)

- **feat(seed+dashboard):** Generador determinista de semana de datos en `seed-supabase.mjs` (RNG mulberry32 sembrado): 1008 lecturas (12/equipo/día, 24–31 jul 2026), 168 IoT (farmacia) + manuales, `snapshot_min/max_temp` HACCP, `reading_type`, `recorded_by_profile/staff` reales. 8 incidentes (3 abiertos, 5 resueltos, acción ≥20 chars). Idempotente por upsert. `getReadingsLast7Days` + lib `dashboardMetrics.ts` (tendencia, cumplimiento %, promedios, min/max del día) + cards nuevas en `DashboardPage` (LineChart Recharts + Progress). 10 tests.
- **fix(deploy):** Quitar `cleanUrls` de `vercel.json` — entra en conflicto con el rewrite SPA a `/index.html` (la doc de Vercel pide destination sin extensión con `cleanUrls` activo), causando 404 del edge (`Code: NOT_FOUND / ID: gru1::...`) al recargar rutas internas (`/admin/*`, `/readings/history`).
- **feat(reports):** Tabla compacta (max-h + scroll, padding reducido) + `VariationCharts` (evolución por equipo multi-línea + banda diaria promedio/mín/máx). Lib `reportCharts.ts` (6 tests).
- **style(reports):** Orden por secciones y cards compactas; quitar `TemperatureChart` grande y su placeholder (el PDF captura ahora el card de evolución como imagen); card de incidentes por día (nuevo); el gráfico de evolución usa el **nombre** del equipo como clave (antes el id/código).
- **fix(reports):** `listEquipmentByOrganization` (patrón ADR-007, 2 queries) para mostrar nombre y rango reales de equipos cuando el reporte usa "todas las sedes" (antes `name: id`). Verificado contra BD real con `owner@restonorte.cl` (RLS ok).

### Decisiones de diseño aplicadas (vivas)

- **Seed determinista (no aleatorio puro):** `mulberry32` con seed fijo → re-ejecutar el seed produce exactamente los mismos ids y valores (upsert idempotente estable). Sin dependencias nuevas.
- **Preservar los 16 ids de lecturas del seed v1 como "espejo"** de la semana (sus incidentes originales siguen apuntando a lecturas existentes; no quedan huérfanos).
- **Sedes en límite de plan:** el trigger `check_location_limit` (BEFORE INSERT) bloquea el upsert de una sede existente cuando la org ya está en el límite. El seed hace UPDATE si la sede existe y INSERT si no — sin tocar BD.
- **UUID válidos en ids generados:** el 4to grupo del UUID debe tener 12 chars; se codifica `7d` + suffix(8) + slot(2). Bug encontrado en el seed (grupo de 13 chars) y corregido.
- **Métricas 7d como lib pura** (`dashboardMetrics.ts`): `computeTrend`, `computeCompliancePercent` (usa snapshot HACCP con fallback al rango actual), `computeAveragesByEquipment`, `computeMinMaxToday`. `now` inyectable para tests deterministas.
- **`listEquipmentByOrganization` en vez de fallback `name: id`:** cuando el reporte no filtra por sede, se traen los equipos de la org (2 queries ADR-007) para mostrar nombre y rango reales en selector, leyenda y tooltip.
- **`buildEquipmentSeries` omite equipos sin lecturas:** evita series vacías que ensucian el chart multi-línea.
- **Gráfico de incidentes por día:** `buildIncidentDailySeries` separa abiertos/resueltos por día.
- **PDF sin `TemperatureChart`:** el `chartRef` ahora apunta al card de "Evolución por equipo" (forwardRef), que se embebe como PNG en el PDF.

### Tests añadidos

- `tests/unit/dashboardMetrics.test.ts` — 10 tests: buckets de 7 días, tendencia con días vacíos, cumplimiento con/ sin snapshot, promedios por equipo ordenados, min/max del día.
- `tests/unit/reportCharts.test.ts` — 9 tests: series por equipo (orden, filtro, omisión de vacíos, nombre), banda diaria (avg/min/max, orden), incidentes por día (agrupación, orden, vacío).

### Riesgos / pendientes técnicos

- **Tooling de tests:** persiste el bug preexistente `React.act is not a function` (React 19.2.7 + RTL 16.3) — 367 tests de hooks/componentes siguen fallando. `useReport.test.ts` (16) también, aunque los mocks ya se actualizaron (listEquipmentByOrganization). No introducido en esta sesión.
- **Lint:** persisten 5 errores preexistentes de setState en effect (`useIncidents.ts`, `useLocationsManagement.ts`, `ReadingsHistoryPage.tsx`). Se corrigieron los 2 nuevos de `useDashboardData` (mismos patrones, con guards derivados en vez de setState en el effect).
- **H-002:** rotar anon key (recomendación, no bloqueante).
- **Seed `SEED_ENV`:** para correr contra la BD real hay que usar `SEED_ENV=prod` (el guard de ambiente bloquea URLs que no parecen dev/staging). Autorizado por César en esta sesión.
- **Chunk size:** `ReportsPage` creció por Recharts (warning de Vite, preexistente, ~163 kB gzip). Lazy-load ya lo aísla.

### Pendientes (siguiente tarea lógica)

1. **Resolver el tooling de tests** (`React.act is not a function`) — es la deuda que impide tener la suite completa en verde y validar hooks/componentes con CI.
2. **Cerrar los 5 errores de lint** preexistentes (setState en effect).
3. **Validar visual en preview de Vercel** el rediseño de reportes y las métricas del dashboard (pendiente de QA manual de César tras el redeploy).
4. **V2 (fuera de alcance):** flujo de aprobación de alta de empresas (`pending`), gestión de usuarios cross-tenant (Edge Function + service role), billing/pagos, Realtime con filtro de canal, desglose de cumplimiento por equipo (follow-up documentado en TASK-011).

---

## Pendientes

> No hay tareas de estabilización pendientes en el backlog activo para el Hito V1.0.0. El histórico de tareas terminadas se encuentra en `files/TASKS_HISTORY.md`.
